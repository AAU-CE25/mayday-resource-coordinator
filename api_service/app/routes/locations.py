...
@router.post("/address", response_model=LocationResponse, dependencies=[Depends(require_role(["AUTHORITY"]))])
...
@router.get("/geocode", response_model=LocationResponse, dependencies=[Depends(require_role(["AUTHORITY"]))])
...
@router.get("/", response_model=list[LocationResponse], dependencies=[Depends(require_role(["AUTHORITY"]))])
def read_locations():
    return LocationLogic.get_locations()

@router.get("/{location_id}", response_model=LocationResponse, dependencies=[Depends(require_role(["AUTHORITY"]))])
...
@router.put("/{location_id}", response_model=LocationResponse, dependencies=[Depends(require_role(["AUTHORITY"]))])
...
@router.delete("/{location_id}", dependencies=[Depends(require_role(["AUTHORITY"]))])
